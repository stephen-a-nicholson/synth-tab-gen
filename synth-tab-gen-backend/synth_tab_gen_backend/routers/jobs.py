...
from synth_tab_gen_backend.models import GenerationConfig, APIResponse, OutputFormat
...
async def generate_data_task(job_id: str, model_id: str, config: GenerationConfig):
    ...
    try:
        ...
        dataset_id = storage.store_dataset(
            filename=f"synthetic_data_{model_id}.csv",
            df=synthetic_data
        )
...
def get_generated_data(dataset_id: str, format: OutputFormat = OutputFormat.CSV):
