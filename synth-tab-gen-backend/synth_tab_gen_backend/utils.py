...
def introduce_missing_values(df: pd.DataFrame, percentage: float) -> pd.DataFrame:
    ...
    indices = [(i, j) for i in range(len(df_copy)) for j in range(len(df_copy.columns))]
    
    # Randomly select cells to nullify
    null_indices = np.random.choice(len(indices), cells_to_nullify, replace=False)
...
def introduce_duplicates(df: pd.DataFrame, percentage: float) -> pd.DataFrame:
    ...
    duplicate_indices = np.random.choice(len(df_copy), rows_to_duplicate, replace=False)
...
def introduce_outliers(df: pd.DataFrame, percentage: float) -> pd.DataFrame:
    ...
    col_indices = np.random.choice(len(numeric_columns), cells_to_modify, replace=True)
    ...
    for i, j in zip(row_indices, col_indices):
        ...
        df_copy.iloc[i, df_copy.columns.get_loc(col)] = mean + outlier_factor * std
