...
page = st.sidebar.selectbox(
    "Navigation", 
    ["Upload Dataset", "Train Model", "Generate Data", "View Datasets", "View Models"]
)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if job_data["status"] == "completed":
                    ...
                    progress_bar.progress(100)
                    status_text.error(f"Task failed: {job_data.get('error', 'Unknown error')}")
                    return None
                else:
                    # Update progress
                    progress = job_data.get("progress", 0)
                    progress_bar.progress(int(progress))
                    status_text.info(f"Status: {job_data['status']} - {progress:.0f}%")
                    time.sleep(1)
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not datasets:
                ...
                # Dataset selection
                dataset_options = {f"{d['filename']} ({d['rows']} rows)": d["dataset_id"] for d in datasets}
                ...
                model_type = st.selectbox(
                    "Select Model Type",
                    ["CTGAN (Deep Learning)", "TVAE (Deep Learning)", "GaussianCopula (Statistical)"]
                )
                ...
                with col2:
                    ...
                    if use_gpu:
                        st.info("GPU acceleration can provide 5-15x faster training")
                    ...
                    with st.spinner("Starting model training..."):
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                st.info(f"Training started with job ID: {job_id}")
                                
                                # Poll for job status
                                result = poll_job_status(job_id)
                                
                                if result:
                                    st.success(f"Model trained successfully! Model ID: {result['model_id']}")
                            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not models:
                ...
                # Model selection
                model_options = {f"{m['model_type']} Model ({m['created_at']})": m["model_id"] for m in models}
                ...
                with col1:
                    num_rows = st.number_input("Number of Rows to Generate", 100, 1000000, 10000)
                    ...
                    with st.spinner("Starting data generation..."):
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                st.info(f"Generation started with job ID: {job_id}")
                                ...
                                if result:
                                    ...
                                    response = requests.get(f"{API_URL}/jobs/data/{dataset_id}")
                                    if response.status_code == 200:
                                        preview_data = response.json()["data"]["preview"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not datasets:
                ...
                dataset_options = {f"{d['filename']} ({d['rows']} rows)": d["dataset_id"] for d in datasets}
                selected_dataset = st.selectbox("Select Dataset for Details", list(dataset_options.keys()))
                dataset_id = dataset_options[selected_dataset]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not models:
                ...
                model_options = {f"{m['model_type']} Model ({m['created_at']})": m["model_id"] for m in models}
                selected_model = st.selectbox("Select Model for Details", list(model_options.keys()))
                model_id = model_options[selected_model]
